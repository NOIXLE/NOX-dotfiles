...
import html  # Add to imports at the top

def escape_pango(text: str) -> str:
    """
    Escape characters that are unsafe in Pango markup.
    """
...
def create_tooltip_text(
    ...
    if artist or track:
        tooltip += f'<span foreground="{track_color}"><b>{escape_pango(track)}</b></span>\n<span foreground="{artist_color}"><i>{escape_pango(artist)}</i></span>\n'
...
def write_output(track, artist, playing, player, tooltip_text):
    ...
    if track and not artist:
        output_text = f"{prefix} <b>{escape_pango(track)}</b>"
    elif track and artist:
        output_text = f"{prefix} <b>{escape_pango(track)}</b>  <i>{escape_pango(artist)}</i>"
    else:
