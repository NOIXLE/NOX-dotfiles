...
# Don't ask me about this, thank you HYDE Project for the script! (I commented some stuff out that we are not using)
...
notify_vol() {
    ...
    #[[ "${isNotify}" == true ]] && notify-send -a "Spotify" -r 8 -t 800 -i "${ico}" "${vol}${bar}" "${nsink}"
    [[ "${isNotify}" == true ]] && notify-send -a "Spotify" -r 8 -t 800 "Volume - ${vol}%" "${nsink}"
}
